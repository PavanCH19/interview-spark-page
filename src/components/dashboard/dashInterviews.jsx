...
import axios from 'axios';
import {
    Calendar, Play, Clock, ChevronRight
} from 'lucide-react';
...
const DashInterviews = () => {
        ...
        const fetchSessions = async () => {
            try {
                ...
                const response = await axios.post(
                    "http://localhost:3000/api/interview/recent-sessions",
                    { domain: "ai_ml" },
                    {
                        headers: {
    ...
    // --------------------------------------------------
    // 🔥 Open Modal and Pass Selected Session
    // --------------------------------------------------
        ...
        <>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-8 lg:p-10 text-white shadow-xl">
                    ...
                    <button
                        className="px-8 py-3 bg-white text-indigo-600 hover:bg-gray-100 rounded-xl font-bold flex items-center space-x-2 transition-all shadow-lg hover:shadow-xl"
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6">
                            {preparationPaths.map(session => (
                                <div
                                    key={session.id}
                                    className="h-full rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-5 shadow-sm hover:shadow-md transition"
                                >
                                    <div className="space-y-3">
                                        <h4 className="text-base font-bold text-gray-900 truncate">{session.domain}</h4>
                                        <p className="text-xs text-gray-600 font-medium truncate">
                                            {session.company} • Started {session.started}
                                        </p>
                                    </div>
                                    ...
                                    <button
                                        className="mt-4 w-full py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 text-white rounded-xl text-sm font-semibold shadow hover:shadow-lg"
                                        onClick={() => navigate("/interview")}
                                    >
                            ...
                            <tbody>
                                {recentSessions.map((session) => (
                                    <tr key={session._id} className="border-b hover:bg-gray-50">
                                        ...
                                        <td className="py-4 px-4">
                                            <span className="px-3 py-1.5 bg-indigo-100 text-indigo-800 rounded-full text-xs font-bold">
                                                {session.score}%
                                            </span>
                                        </td>

                                        <td className="py-4 px-4">
                                            <span className="px-3 py-1.5 bg-emerald-100 text-emerald-800 rounded-full text-xs font-bold">
                                                {session.status}
                                            </span>
                                        </td>
